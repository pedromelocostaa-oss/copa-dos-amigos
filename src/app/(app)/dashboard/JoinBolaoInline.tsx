...
export default function JoinBolaoInline({ userId }: { userId: string }) {
  ...
  async function handleJoin() {
    ...
    setLoading(true)
    const { data: bolao } = await supabase.from('boloes').select('id,name').eq('code', c).single()
    if (!bolao) { toast('Código inválido.', 'error'); setLoading(false); return }
    await supabase.from('bolao_members').upsert(
      { bolao_id: bolao.id, user_id: userId, payment_status: 'pendente' },
      { onConflict: 'bolao_id,user_id' }
    )
    toast(`Entrou em "${bolao.name}"!`, 'success')
    router.refresh()
    setCode('')
    setOpen(false)
    setLoading(false)
    ...
    return (
      <div className="flex gap-2">
        ...
        <Link href="/onboarding"
          className="flex items-center justify-center gap-2 border-2 border-dashed border-gray-200 hover:border-green-400 text-gray-500 hover:text-green-600 rounded-2xl px-4 py-4 text-sm font-medium transition">
          ➕
        </Link>
      </div>
    )
  ...
  return (
    <div className="bg-white rounded-2xl border-2 border-green-400 p-4 space-y-3">
      <div className="flex items-center justify-between">
        ...
        <button onClick={() => { setOpen(false); setCode('') }} className="text-gray-400 hover:text-gray-600 text-lg leading-none">×</button>
      </div>
      <input
        value={code}
